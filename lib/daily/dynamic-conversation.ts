...
export function buildDynamicConversationActivity(input: {
  ...
  return [
    ...
    input.turnIndex === 0
      ...
      : input.turnIndex >= 2
        ? "讲解结构：孩子已经给过一部分答案，本轮优先用 narrate 做阶段小结和儿童化科普；不要直接进入下一个追问。"
        : "追问结构：先用 narrate 接住孩子的一个关键词；再用一个输入工具只追一个问题。",
    "对话路线：先听孩子的初步想法 -> 再追一个理由或线索 -> 林老师总结刚才的问题和孩子答案，并讲一个小概念 -> 如有必要再换一个小条件检查想法 -> 最后收住。",
    "阶段讲解要求：得到部分答案后，narrate 必须包含 3 件事：刚才我们在想什么、孩子说到的一个关键词、一个生活化小知识或小方法。",
    "阶段讲解轮可以只调用 narrate；如果这一小段已经自然讲清楚，就用 narrate + end_activity 收住，不要再追加输入工具。",
    "禁止连续追问：如果上一轮已经追问过，这一轮不能只继续问“为什么/下一步/还有吗”，必须先总结和讲解。",
